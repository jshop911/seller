...
const SignUpScreen = ({ navigation }) => {
	...
	async function signUp() {
		...
		if (
			...
			return;
		} else if (value.phoneNum.length < 10) {
			setValue({
				...value,
				error: "Please input your valid phone number \nex. 9*********",
			});
			return;
			...
			app
				...
				.then((response) => {
					...
					const data = {
						...
						photoURL:
							selectedImage ||
							"https://t3.ftcdn.net/jpg/02/16/42/22/360_F_216422244_E5Dci50kkesrmrysk4kttonvRDaRxLt1.jpg",
					};
					...
					usersRef
						...
						.then(() => {
							...
							Alert.alert("Welcome to Jshop! Happy Selling 😀");
						})
						.catch((error) => {
			...
			<KeyboardAwareScrollView>
				<View style={tw`flex-1 bg-[#e0e0e0]`}>
					<Image
						...
						source={require("../assets/images/signup-banner.jpeg")}
					/>
					<Text style={tw`text-4xl p-3 text-center text-gray-700 font-bold`}>
						Register now!
					</Text>

					{!!value.error && (
						<View style={tw`bg-red-800`}>
					...
					<View style={tw`px-4`}>
						...
						<View style={tw`ml-2 py-2`}>
							<Text style={tw`text-sm text-gray-500`}>
								By clicking sign up button, you can confirm that you accept our
						...
						<CustomBtn text="Sign Up" onPress={signUp} />
					</View>
					{/* Sign up/ or registration button */}
					<Text style={tw`text-center text-sm py-13`}>
